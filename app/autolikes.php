...
if($limit < 101){
    ...
    if($check_token['id']){
        $uid = $check_token['id'];
        $check = $page->sqlite_single_row('select * from block_time where type = "autolike" and (username = "'.$_SESSION['info']['username'].'" or ip ="'.$_SERVER['REMOTE_ADDR'].'" or uid = "'.$uid.'")');
        if(sizeof($check) > 0){
            if((time() > ($check['time'] + 900)) == false){//block like 15m
                ...
                $block = 0;
                $page->sqlite_query('delete from block_time where type = "autolike" and (username ="'.$_SESSION['info']['username'].'" or ip = "'.$_SERVER['REMOTE_ADDR'].'" or uid = "'.$uid.'")');
            }
        }else{
            $block = 0;
        }
        if($block != 1){
            ...
            $page->sqlite_query('insert into block_time (time,type,username,ip,uid) VALUES ("'.time().'","autolike","'.$_SESSION['info']['username'].'","'.$_SERVER['REMOTE_ADDR'].'","'.$uid.'")');
