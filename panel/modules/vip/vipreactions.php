...
$page = new core();
$page->create_content('modules/vip/vipreactions.html');
...
if(sizeof($menu) > 0){
    foreach($menu as $r){
        ...
        $row_arr[] = array($r['id'],$r['uid'],$r['buy_time'],$r['time'].' Month',$checked,$r['username'],date('Y-m-d',strtotime($r['buy_time'] . '+'.$r['time'].' month')),$r['type'],$r['package'].' - ('.$config['vip'.$r['package']].' Reactions)');
    }   
}else{
